...
mtx <- readRDS(file= "/suffolk/WorkGenomicsD/ec474/demultiplex/mtx_umis_clara_0.95_1000_singlets.rds")

DefaultAssay(mtx) <- "ATAC"
mtx
# An object of class Seurat
# 134811 features across 3188 samples within 3 assays
...
# load mgatk output
mito.data <- ReadMGATK(dir="/suffolk/WorkGenomicsD/ec474/mgatk/final")
# Reading allele counts
# Reading metadata
# Building matrices

# mito.data$ tab tab
# mito.data$counts     mito.data$depth      mito.data$refallele
...
het <- callHeteroplasmyByPosition(mtx, 3243, 5)
colnames(het)
...
head(colnames(mtx))
# [1] "AAACATGCAGTTATGT-1" "AAACCAACAAACCCTA-1" "AAACCAACACTGACTA-1"
# [4] "AAACCAACAGTCTAGC-1" "AAACCAACATCCATCT-1" "AAACCGAAGCATTTCT-1"

#Plot depth, and optimise the depth cut off, violn or density plot
#Published literature set this btw 10-20

#Check CB are in the same order before using AddMetaData
ifelse(rownames(het) == colnames(mtx), 
  print("Hurray"), 
  print("No way"))
...
library(ggplot2)
VlnPlot(mtx, "Heteroplasmy", pt.size = 0.1, split.by="HTO_maxID") + labs(title="scHeteroplasmy by hash") + scale_y_continuous(labels = scales::percent)
ggsave(filename="vlnplot_scHeteroplasmy_depth_5_by_hash.jpg", path="/suffolk/Homes/ec474/scripts/mgatk/figures")

table(mtx$HTO_maxID)
# NPC-HASH1 NPC-HASH2 NPC-HASH3  unmapped
#       221       159       254      2554

table(mtx$HTO_classification.global)
# Singlet
#    3188
...
saveRDS(mtx, file= "/suffolk/WorkGenomicsD/ec474/mgatk/mtx_umis_clara_0.95_1000_singlets_RNA_ATAC_QC_mgatk.rds")
...
Idents(mtx) <- 'HTO_maxID'
VlnPlot(mtx, "mtDNA_depth", pt.size = 0.1, split.by = 'HTO_maxID') + labs(title="average mtDNA depth by hash")
ggsave(filename="vlnplot_mtDNA_depth_by_hash.jpg", path="/suffolk/Homes/ec474/scripts/mgatk/figures")
#Remove log scale for now + scale_y_log10()
